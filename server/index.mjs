...
io.on('connection',socket=>{
  ...
  socket.on('room:start',(_,reply=()=>{})=>{const room=liveRooms.get(socket.data.roomCode);if(!room||room.leaderId!==user.id)return reply({ok:false,error:'Yalnızca masa lideri başlatabilir.'});const guests=[...room.players.values()].filter(p=>p.id!==room.leaderId);if(!guests.length)return reply({ok:false,error:'Başlamak için en az bir arkadaş gerekli.'});if(guests.some(p=>!p.ready||!p.online))return reply({ok:false,error:'Bütün oyuncular hazır ve çevrimiçi olmalı.'});room.status='playing';io.to(room.code).emit('room:started',roomView(room));io.to(room.code).emit('room:update',roomView(room));reply({ok:true})});
