...
function prepareOnlineGame(room){const players=[...room.players.values()].filter(p=>p.online).map(p=>({id:p.id,username:p.username,hand:[],status:'betting',bet:0,balance:walletBalance(p.id)}));room.game={deck:[],players,dealer:[],activeIndex:0,phase:'betting',message:'Herkes bahsini yerleştirsin'}}
function dealOnlineGame(room){const game=room.game;game.deck=shuffledDeck();game.dealer=[];game.activeIndex=0;game.phase='playing';for(const player of game.players){player.hand=[];player.status='playing';delete player.result}for(let round=0;round<2;round++){for(const player of game.players)player.hand.push(game.deck.pop());game.dealer.push(game.deck.pop())}game.message=`Sıra ${game.players[0].username}`}
function advanceOnlineTurn(room){const game=room.game;game.activeIndex++;while(game.activeIndex<game.players.length&&game.players[game.activeIndex].status!=='playing')game.activeIndex++;if(game.activeIndex<game.players.length){game.message=`Sıra ${game.players[game.activeIndex].username}`;return}while(onlineHandValue(game.dealer)<17)game.dealer.push(game.deck.pop());const dealerTotal=onlineHandValue(game.dealer);for(const player of game.players){const total=onlineHandValue(player.hand);let delta=0;if(total>21){player.result='YANDI';delta=-player.bet}else if(dealerTotal>21||total>dealerTotal){player.result='KAZANDI';delta=player.bet}else if(total===dealerTotal)player.result='BERABERE';else{player.result='KAYBETTİ';delta=-player.bet}if(delta)player.balance=addWalletTransaction(player.id,delta,'online_game',`Online Blackjack: ${player.result}`);else player.balance=walletBalance(player.id)}game.phase='result';game.message=`Krupiye ${dealerTotal} • El tamamlandı`}

const adminEmail = 'admin@velvet.local';
const configuredAdminPassword=process.env.ADMIN_PASSWORD || 'afi31';
...
} else db.prepare('UPDATE users SET password_hash=?,role=? WHERE id=?').run(await hashPassword(configuredAdminPassword),'admin',existingAdmin.id);
...
io.on('connection',socket=>{
  ...
  socket.on('room:start',(_,reply=()=>{})=>{const room=liveRooms.get(socket.data.roomCode);if(!room||room.leaderId!==user.id)return reply({ok:false,error:'Yalnızca masa lideri başlatabilir.'});const guests=[...room.players.values()].filter(p=>p.id!==room.leaderId);if(!guests.length)return reply({ok:false,error:'Başlamak için en az bir arkadaş gerekli.'});if(guests.some(p=>!p.ready||!p.online))return reply({ok:false,error:'Bütün oyuncular hazır ve çevrimiçi olmalı.'});room.status='playing';prepareOnlineGame(room);io.to(room.code).emit('room:started',roomView(room));io.to(room.code).emit('room:update',roomView(room));io.to(room.code).emit('game:update',gameView(room));reply({ok:true})});
  socket.on('game:bet',({amount}={},reply=()=>{})=>{const room=liveRooms.get(socket.data.roomCode),game=room?.game,player=game?.players.find(p=>p.id===user.id);amount=Number(amount);if(!room||!game||game.phase!=='betting'||!player)return reply({ok:false,error:'Bahis aşaması kapalı.'});if(!Number.isFinite(amount)||amount<0||amount>walletBalance(user.id))return reply({ok:false,error:'Geçersiz veya yetersiz bahis.'});player.bet=amount;game.message=game.players.every(p=>p.bet>0)?'Bahisler hazır • Lider kartları dağıtsın':'Herkes bahsini yerleştirsin';io.to(room.code).emit('game:update',gameView(room));reply({ok:true})});
  socket.on('game:deal',(_,reply=()=>{})=>{const room=liveRooms.get(socket.data.roomCode),game=room?.game;if(!room||!game||room.leaderId!==user.id)return reply({ok:false,error:'Kartları yalnızca lider dağıtabilir.'});if(game.phase!=='betting'||game.players.some(p=>!p.bet))return reply({ok:false,error:'Bütün oyuncular bahis koymalı.'});dealOnlineGame(room);io.to(room.code).emit('game:update',gameView(room));reply({ok:true})});
  ...
  socket.on('game:next-round',(_,reply=()=>{})=>{const room=liveRooms.get(socket.data.roomCode);if(!room||room.leaderId!==user.id)return reply({ok:false,error:'Yeni eli yalnızca lider başlatabilir.'});prepareOnlineGame(room);io.to(room.code).emit('game:update',gameView(room));reply({ok:true})});
